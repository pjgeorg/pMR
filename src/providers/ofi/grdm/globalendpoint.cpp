...
void pMR::OFI::GlobalEndpoint::postSend(Tagged &message)
{
    ...
    if(cThreadLevel >= ThreadLevel::Multiple)
    {
        ...
        postSendRequest(
            mEndpoint, message, mDomain.checkInjectSize({message.getLength()}));
    }
    else
...
void pMR::OFI::GlobalEndpoint::postRecv(Tagged &message)
{
#ifndef OFI_RDM_GLOBAL_THREADSAFE
    if(cThreadLevel >= ThreadLevel::Multiple)
    {
...
void pMR::OFI::GlobalEndpoint::postWrite(RMA &message)
{
#ifndef OFI_RDM_GLOBAL_THREADSAFE
    if(cThreadLevel >= ThreadLevel::Multiple)
    {
