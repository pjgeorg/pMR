...
if(PROVIDER.OFI)
    ...
    if(OFI.EP STREQUAL "RDM")
        if(OFI.RDM.GLOBAL)
            set(OFI_CAPS "${OFI_CAPS} RDMGlobal")
            add_definitions(-DOFI_RDM_GLOBAL)
            if(OFI.RDM.GLOBAL.THREADSAFE)
                set(OFI_CAPS "${OFI_CAPS} RDMGlobalThreadSafe")
    ...
    if(OFI.RMA)
        ...
        if(OFI.RMA.EVENT)
            ...
            if(OFI.RMA.EVENT.NONZERO)
                set(OFI_CAPS "${OFI_CAPS} RMAEventNonZero")
        ...
        if(OFI.RMA.CONTROL)
            set(OFI_CAPS "${OFI_CAPS} RMAControl")
            add_definitions(-DOFI_RMA_CONTROL)
        endif()
    else()
        if(OFI.NO.CONTROL)
            set(OFI_CAPS "${OFI_CAPS} NoControl")
...
if(PROVIDER.PSM2)
    ...
    if(PSM2.WRAPPER)
        ...
        set(PSM2_OPTS "${PSM2_OPTS} Wrapper")
    endif()

    if(PSM2.WRAPPER.BLOCK)
        add_definitions(-DPSM2_WRAPPER_BLOCK)
        set(PSM2_OPTS "${PSM2_OPTS} WrapperBlock")
    endif()
    ...
    set(PROVIDER_STRING "${PROVIDER_STRING} PSM2")
    INFO("PSM2 Library: ${PSM2.LIB} with options:${PSM2_OPTS}")
endif()
...
if(PROVIDER.SCIF)
    ...
    set(PROVIDER_STRING "${PROVIDER_STRING} SCIF")
endif()
...
if(PROVIDER.VERBS)
    ...
    if(VERBS.RDMA)
        ...
        if(VERBS.RDMA.CONTROL)
            set(VERBS_CAPS "${VERBS_CAPS} RDMAControl")
